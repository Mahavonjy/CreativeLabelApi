...
class ReservationSchema(ValidateSchema):
    ...
    lastname = fields.Str(nullable=True)
    email = fields.Str(nullable=True)
    invoice = fields.Str(nullable=True)
    city = fields.Str(nullable=True)
    postal_code = fields.Int(nullable=True)
    phone = fields.String(nullable=True)
    services_id = fields.Int(nullable=False)
    total_amount = fields.Float(required=True)
    event_date = fields.DateTime(required=True)
    artist_owner_id = fields.Int(nullable=False)
    stripe_token = fields.Dict(nullable=True)
    payment_history_id = fields.Int(nullable=False)
    auditor_who_reserve_id = fields.Int(nullable=False)
    options_id_list = fields.List(fields.Int(), nullable=True)
