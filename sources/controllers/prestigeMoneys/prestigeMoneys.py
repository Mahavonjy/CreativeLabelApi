...
from sources.models.artists.history.history import ArtistHistorySchema
from sources.models.users.user import User, UserSchema
from sources.controllers import convert_dict_to_sql_json
from sources.controllers.medias.mediaControllers import get_one_media
from sources.controllers import create_artist_new_story_dict
...
def prestige_data(ser_user_sender_id, ser_user_recipient_id, prestige, song_id):
    ...
    return dict(
        ...
        sender_id=ser_user_sender_id, prestige=defaultDataConf.prestige_allowed_type[prestige]
    )
...
def send_prestige_email(prestige, song_id, user_connected_model, user_connected_schema):
    ...
    recipient_story = month_story_schema.dump(user_recipient_story_model)
    if not recipient_story['months_story'].get(str(now)):
        recipient_story['months_story'].update(create_artist_new_story_dict())
    recipient_story['months_story'][str(now)]['prestige'].append(defaultDataConf.prestige_allowed_type[prestige])
